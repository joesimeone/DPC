...
county_for_knitr <- dpc_clean %>% mutate(year = as.numeric(year)) %>% 
                                  filter(year %in% 2018:2022) %>%
                                  mutate(year =as.character(year)) %>% 
  
                                  mutate(county = case_when(county == "K" ~ "Kent",
                                                            county == "SUS" ~ "Sussex",
                                                            county == "NCC" ~ "New Castle")) %>% 
  
                                  group_by(year, county) %>% 
                                  summarise(population = sum(population_est))  
...
knitr::kable(county_for_knitr, "html", caption = "<center><strong>Table 1.0: Population Estimates, By County, 2018-2022</strong></center>", escape = FALSE) %>% kable_styling(bootstrap_options = c("striped", "hover", "responsive", position = "left")) %>% 
                    pack_rows(index = table(county_for_knitr$year)) %>% 
                    footnote(general = c("<i>Estimates as of October 2022</i>"), escape = FALSE)
